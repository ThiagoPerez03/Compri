...
@csrf_exempt
def api_de_descompresion(request):
    """
    Endpoint para descomprimir un archivo .zip previamente generado.
    """
    if request.method != 'POST':
    ...
    try:
        with zipfile.ZipFile(archivo_zip, 'r') as zf:
            # Asumimos que el ZIP contiene un .json como en tu lógica original
            # Si el ZIP contiene los .txt, la lógica de descompresión cambiaría
